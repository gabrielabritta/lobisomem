...
interface MayorReelectionProps {
  players: Player[]
  config: GameConfig
  previousMayorName: string
  onReelectionComplete: (mayorId: string) => void
}

export default function MayorReelection({ 
  players, 
  config, 
  previousMayorName, 
  onReelectionComplete 
}: MayorReelectionProps) {
  ...
  const handleContinue = () => {
    if (votingResult?.winner) {
      onReelectionComplete(votingResult.winner)
    }
  }
      ...
      <div className="card">
        <div className="space-y-6 text-center">
          ...
          <div className="bg-primary-900/30 border border-primary-700 rounded-lg p-6">
            <p className="text-lg">
              <strong>Novo Prefeito Eleito:</strong> {getVotingResultText()}
            </p>
            {votingResult.tied && (
    ...
    <div className="card">
      <div className="space-y-6">
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-4">👑 Nova Eleição de Prefeito</h2>
